...
public partial class FPSteamManager : Node {
	...
	public Lobby? currentLobby; public ulong GetLobbyHostSteamID() { return currentLobby.Value.Owner.Id.Value; }
	...
	public ulong SetupSteam() {
		...
		try {
			GD.Print("Loading FP");
			SteamClient.Init(2478900, true); //2478900
	...
	private void SteamMatchmaking_OnLobbyMemberJoined(Lobby _lobby, Friend friend) {
		...
		if (GetTree().CurrentScene.Name.Equals(pregameSteamLobby)) {
			MPManager.Call("AddNewLobbyUsername", friend.Name, friend.Id.Value);
		}
	...
	private void SteamMatchmaking_OnLobbyEntered(Lobby _lobby) {
		...
		GetTree().ChangeSceneToFile(pregameSteamLobby);
