...
func start() -> void:
	...
	if not multiplayer.is_server():
		...
		print("_request_timestamp in network-time-synchronizer.gd")
		#_request_timestamp().rpc_id( Steam.getLobbyOwner( SteamManager.currentLobby) )
		#_request_timestamp.rpc_id( get_tree().get_multiplayer().multiplayer_peer.get_peer_id_from_steam64( Steam.getLobbyOwner( SteamManager.currentLobby ) ) )
